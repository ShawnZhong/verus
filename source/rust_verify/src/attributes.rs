    ...
    for (prefix, span, attr) in attrs_to_trees(attrs)? {
        ...
        match prefix {
            AttrPrefix::Verifier => match &attr {
                ...
                AttrTree::Fun(_, arg, None) if arg == "skip_inst_collector" => {
                    v.push(Attr::SkipInstCollector)
                }
                ...
                AttrTree::Fun(_, arg, None) if arg == "ignore_outside_new_mut_ref_experiment" => {
                    v.push(Attr::IgnoreOutsideNewMutRefExperiment)
                }
            ...
            AttrPrefix::Verus(verus_prefix) => match verus_prefix {
                VerusPrefix::Internal => match &attr {
                    ...
                    AttrTree::Fun(_, name, None) if name == "infer_proph" => {
                        v.push(Attr::InferProph)
                    }
...
pub(crate) fn migrate_postconditions_walk_parents<'tcx>(
    ...
    for attr in parse_attrs_walk_parents(tcx, def_id) {
        if let Attr::MigratePostconditionsWithMutRefs(flag) = attr {
            return Some(flag);
        }
    }
    None
...
pub(crate) struct VerifierAttrs {
    ...
    pub(crate) structural_const_wrapper: bool,
    pub(crate) skip_inst_collector: bool,
...
pub(crate) fn get_verifier_attrs_maybe_check(
    ...
    for attr in parse_attrs(attrs, diagnostics)? {
        match attr {
            ...
            Attr::SkipInstCollector => vs.skip_inst_collector = true,
